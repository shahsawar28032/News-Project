...
if($_SESSION["user_role"] == '0'){
  header("Location: {$hostname}/shahsawar/users.php");
}
?>
          ...
          if (mysqli_num_rows($result) > 0) {
              ...
              <form action="<?php $_SERVER['PHP_SELF']; ?>" method ="POST">
                  <div class="form-group">
                      <input type="hidden" name="cat_id"  class="form-control mt-2" value="<?php echo $row['category_id']; ?>">
                  </div>
                  <div class="form-group">
                      <label>category Name</label>
                      <input type="text" name="cat_name" class="form-control mt-2" value="<?php echo $row['category_name']; ?>"  placeholder="" required>
                  </div>
        ...
        <?php
          if(isset($_POST['submit'])){
            ...
            if (mysqli_num_rows($result1)> 0) {
              ...
              $sql1 = "UPDATE category SET category_id='$cat_id', category_name='$category'  WHERE  category_id= '$cat_id'";

              if (mysqli_query($conn,$sql1)){
                // redirect all category page
                header("location: {$hostname}/shahsawar/category.php?ucm=Category Successfully Updated");
              }
            }
          }
          ?>
